...
describe("PS", function () {
    ...
    describe("Test ps workflow", function () {
        let gx1, gy1, bn128, lib, gtilde, y, x, m, gy, gx, h, h2, sigma1, sigma2, ski, sigma1random, sigma2random, c, s,
            signpairing, yk, k, preg, rpairing, u, t, sigma12, tautilde, psContract, sigma1randomk, ymink,
            sigma1random2;
        ...
        it("Setup ", async function () {
            ...
            x = bn128.Fr.random()
            y = bn128.Fr.random()
            gtilde = bn128.G1.timesFr(bn128.G1.one, bn128.Fr.e(preg))
            gx = bn128.G1.timesFr(gtilde, bn128.Fr.e(x))
            gy = bn128.G1.timesFr(gtilde, bn128.Fr.e(y))
            const gsk = (x, y)
        ...
        it("Join ", async function () {
            //joinG
            ski = bn128.Fr.random()
            const tau = bn128.G2.timesFr(bn128.G2.one, bn128.Fr.e(ski))
            tautilde = bn128.G1.timesFr(gy, bn128.Fr.e(ski))
            /*            const tauy = bn128.pairing(tau, gy)
                        const gtau = bn128.pairing(bn128.G1.one, tautilde)
                        expect(bn128.Gt.eq(tauy, gtau)).to.eq(true)*/
            u = bn128.Fr.random()
            sigma1 = bn128.G2.timesFr(bn128.G2.one, bn128.Fr.e(u))
            sigma2 = bn128.G2.timesFr(bn128.G2.add(bn128.G2.timesFr(bn128.G2.one, bn128.Fr.e(x)), bn128.G2.timesFr(tau, bn128.Fr.e(y))), bn128.Fr.e(u))
        ...
        it("Sign", async function () {
            ...
            t = bn128.Fr.random()
            k = bn128.Fr.random()
            sigma1random = bn128.G2.timesFr(sigma1, bn128.Fr.e(t))
            ...
            sigma2random = bn128.G2.timesFr(sigma2, bn128.Fr.e(t))
            ...
            ymink = bn128.G1.neg(bn128.G1.timesFr(gy, bn128.Fr.e(k)))
        ...
        it("Verification hash", async function (){
            ...
            const realc = ethers.utils.solidityKeccak256(["uint", "uint", "uint", "uint", "uint", "uint", "uint", "uint","uint", "uint", "bytes32"], [sigma1randomO[0][1], sigma1randomO[0][0], sigma1randomO[1][1], sigma1randomO[1][0], sigma2randomO[0][1], sigma2randomO[0][0], sigma2randomO[1][1], sigma2randomO[1][0], yminkO[0], yminkO[1], m])
            expect(bn128.Fr.toObject(bn128.Fr.fromObject(realc))).to.eq(bn128.Fr.toObject(c))
        ...
        /*        it("Verif helped", async function () {
                    //verifG helped
                    const lpairingTest = bn128.pairing(bn128.G1.add(yk, gx),sigma1random )
        ...
        it("Verif with solidity verify function", async function () {
            ...
            //const ea1 = bn128.G1.add(bn128.G1.timesFr(gx, bn128.Fr.e(c)), bn128.G1.add(bn128.G1.timesFr(gy, bn128.Fr.e(s)), ymink))
            //const lpairingsol = bn128.pairing(ea1, sigma1random)
            //const sigma2randomc = bn128.G2.timesFr(sigma2random, bn128.Fr.e(c))
            ...
            //c = bn128.Fr.fromObject(ethers.utils.solidityKeccak256(["uint", "uint","uint", "uint", "uint", "uint", "uint", "uint", "bytes32"], [sigma1random2O[0][1],sigma1random2O[0][0], sigma1random2O[1][1],sigma1random2O[1][0],sigma2randomO[0], sigma2randomO[1], yminkO[0], yminkO[1], m]))
/*            const event1 = resVerif2.events[0].args
            expect(sigma1randomO[0][1]).to.eq(event1.a)
            expect(sigma1randomO[0][0]).to.eq(event1.b)
            expect(sigma1randomO[1][1]).to.eq(event1.c)
            expect(sigma1randomO[1][0]).to.eq(event1.d)
